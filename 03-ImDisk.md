...
- **动态分配内存：** 一定要勾选此项，否则浪费内存。  
  只有使用的时候，才真正的占用内存，这样即使是 8GB 内存的电脑，也可以开辟 2GB 作为内存盘，而不至于导致内存非常紧张。
...
3. **动态分配 (Allocate Memory Dynamically)：** 勾选此项，ImDisk 只会占用实际使用的内存，而不会开机就吞掉几 GB 内存。
   ※ 格式化（File System）时推荐选择 NTFS。因为现在很多现代软件在写入临时文件时，会创建非常复杂的权限节点或者大文件，FAT32 可能会遇到单文件 4GB 限制或兼容性问题。
...
- 勾选“Allocate Memory Dynamically”（动态分配内存）  
  修改大小时，务必勾选这个选项。这样设置后，即使你分了 4GB，如果里面只放了 100MB 的临时文件，它在任务管理器里就只占用 **100MB** 的物理内存，而不是一开机就死死吃掉你 4GB 的内存，非常省资源。